...
@section('title', 'Carga Masiva de Postulantes')
@section('page-title', 'Carga Masiva de Postulantes')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">

    {{-- Acceso a la carga de personal/docentes ────────────────────────────── --}}
    <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-xl px-5 py-3">
        <p class="text-sm text-gray-600">¿Buscas cargar docentes o personal?</p>
        <a href="{{ route('admin.importar-personal') }}"
           class="text-sm font-medium text-[#283342] hover:underline">Ir a carga de personal →</a>
    </div>

    @if(! $gestionActiva)
        <div class="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 text-sm text-amber-700">
            No hay una gestión académica abierta. Abra una gestión antes de cargar postulantes.
        </div>
    @endif
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-5">
        ...
        <ol class="list-decimal list-inside space-y-1 text-sm text-blue-700">
            <li>Descargue la plantilla y complete un postulante por fila.</li>
            <li>Suba el archivo <strong>.csv</strong> o <strong>.xlsx</strong>: se registran como <strong>inscritos</strong> de la gestión activa.</li>
            <li>El sistema calcula cuántos grupos se necesitan (máx. {{ 70 }} por grupo).</li>
            <li>Revise el resumen y presione <strong>ACEPTAR</strong> para crear los grupos y distribuir.</li>
        </ol>
        <a href="{{ route('admin.importar-postulantes.plantilla') }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach([
                        ['nombre',          'Sí', 'Nombre(s) del postulante'],
                        ['apellido',        'Sí', 'Apellido(s) del postulante'],
                        ['ci',              'Sí', 'Cédula de identidad (única en el sistema)'],
                        ['telefono',        'No', 'Teléfono de contacto'],
                        ['sexo',            'No', 'M o F'],
                        ['fecha_nacimiento','No', 'Formato AAAA-MM-DD'],
                        ['ciudad',          'No', 'Ciudad de residencia'],
                        ['colegio',         'No', 'Colegio de procedencia'],
                    ] as $col)
                    <tr>
                        <td class="py-2 pr-4 font-mono text-xs text-gray-700">{{ $col[0] }}</td>
                        <td class="py-2 pr-4">
                            <span class="px-2 py-0.5 rounded-full text-xs
                                {{ $col[1] === 'Sí' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600' }}">
                                {{ $col[1] }}
                            </span>
                        </td>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <form method="POST" action="{{ route('admin.importar-postulantes.procesar') }}" enctype="multipart/form-data">
            @csrf
            <div class="mb-5">
                ...
                <div id="drop-zone"
                    ...
                    <p class="text-sm text-gray-500">Arrastra tu archivo aquí o
                        <span class="text-[#283342] font-medium underline">haz clic para seleccionar</span></p>
                    <p class="text-xs text-gray-400 mt-1">Formatos: .csv · .xlsx — Máximo 5 MB</p>
            ...
            <button type="submit" @disabled(! $gestionActiva)
                    class="w-full py-3 rounded-lg text-white font-semibold text-sm transition hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                    style="background-color: #283342;">
                Procesar y calcular grupos
            </button>
        </form>
    </div>
</div>
...
<script>
    ...
    input.addEventListener('change', () => { label.textContent = input.files[0]?.name ?? 'Ningún archivo seleccionado'; });
