            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-3 font-medium text-gray-800">{{ $g->nombre ?? '—' }}</td>
                        <td class="px-6 py-3 text-gray-600 whitespace-nowrap">
                            {{ $g->fecha_ini->format('d/m/Y') }} — {{ $g->fecha_fin->format('d/m/Y') }}
                        </td>
                        <td class="px-6 py-3 text-gray-600">{{ $g->capacidad_maxima ?? '—' }}</td>
                        <td class="px-6 py-3">
                        ...
                        <td class="px-6 py-3">
                            @if($g->estaAbierta())
                                <span class="text-xs text-green-700 bg-green-50 px-2 py-0.5 rounded-full font-medium">Abierta</span>
                            @else
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                @if($g->estaAbierta())
                                    <form method="POST" action="{{ route('admin.gestiones.cerrar', $g) }}" class="inline"
                                          onsubmit="return confirm('¿Está seguro de cerrar esta gestión?\n\nEsta acción bloqueará todas las modificaciones.')">
                                        @csrf @method('PATCH')
                                ...
                                <form method="POST" action="{{ route('admin.gestiones.destroy', $g) }}"
                                      onsubmit="return confirm('¿Eliminar la gestión \'{{ addslashes($g->nombre ?? '#'.$g->idGestion) }}\'? Esta acción no se puede deshacer.')"
                                      class="inline">
                                    @csrf @method('DELETE')
